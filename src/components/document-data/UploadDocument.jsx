...
const UploadDocument = ({
  ...
  const requiredDocConfig = useMemo(() => {
    return REQUIRED_DOCUMENTS.find((doc) => doc.documentName === documentName);
  }, [documentName]);
  ...
  const isValidDate = useCallback((dateString) => {
    return (
      dateString && dateString.trim() !== '' && !isNaN(Date.parse(dateString))
    );
  }, []);
  ...
  return (
    <div className="w-full bg-white px-4 pt-8 pb-12">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <Controller
              ...
              render={({ field }) => (
                <Checkbox
                  {...field}
                  multiple={false}
                  options={[{ label: 'Has Document Number', value: true }]}
                  disabled={isHasDocumentNumberDisabled}
                  value={isHasDocumentNumberDisabled ? true : field.value}
                />
              )}
            />
            ...
            {(hasDocumentNumber || isHasDocumentNumberDisabled) && (
              <Controller
                ...
                rules={{
                  ...
                  validate: (value) =>
                    value?.trim() !== '' || 'Document Number cannot be empty',
                  pattern: {
                    value: /^[a-zA-Z0-9-]+$/,
                    message:
        ...
        <button
          type="submit"
          disabled={uploadPending || updatePending || isSubmitting}
