...
function MedicationCard({ medication, participantId }) {
  ...
  const medicationState = medication.status === "Completed" ? "Administrated" : medication.status === "Refused" ? "Refused" : "Not administrated"
        ...
        <div className="space-y-2">
          ...
          <div>
            {medication.type === 'medication' && !medication.administratedAt ? (
              <>Administration Time: {formatDate(medication.scheduledDate)} at {formatTime12Hour(medication.scheduledTime)}</>
            ) : (
          ...
          {medication.administratedAt ? (
            <span>
              {medicationState} At:{' '} {format(medication.administratedAt, "HH:mm a, MMM dd, yy")}
            </span>
          ) : (
            <></>
          )}
