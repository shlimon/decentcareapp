...
const ConflictOfInterestForm = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (data.staffParticipants.length > 0) {
      data.staffParticipants.forEach((participantId) => {
        formData.append('staffParticipants[]', participantId);
      });
    }
    ...
    try {
      ...
      if (response?.data?.success) {
        ...
        methods.reset();
        navigate('/forms/conflict-of-interest');
      }
    } catch (error) {
  ...
  return (
    <div className="py-8 px-4 max-w-xl mx-auto">
      <NavigateButton
        navigateUrl="/forms/conflict-of-interest"
        title="Back to my conflicts"
        icon={ArrowLeft}
        iconPosition="left"
      />
      <div>
                  ...
                  options={[
                    {
                      value: true,
                      label:
                        'I confirm that the information provided is true and complete to the best of my knowledge. I understand that I must notify Decent Care if this situation changes.',
                    },
                  ]}
