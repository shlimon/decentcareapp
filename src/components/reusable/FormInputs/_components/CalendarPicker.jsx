...
export function CalendarPicker({
  ...
  useEffect(() => {
    ...
    const calculatePosition = () => {
      ...
      const SPACING_GAP = -6; // Changed from 4 to -6 to move 10px closer
      ...
      const requiredSpaceWithGap = CALENDAR_HEIGHT + 4; // Keep original gap for space check

      // vertical positioning
      if (spaceBelow >= requiredSpaceWithGap) {
        top = rect.bottom + window.scrollY + SPACING_GAP;
      } else if (spaceAbove >= requiredSpaceWithGap) {
        top = rect.top + window.scrollY - CALENDAR_HEIGHT - SPACING_GAP;
      } else {
