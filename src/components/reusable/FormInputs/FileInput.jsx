...
import ImageCropper from '@components/ui/FormInputs/_components/ImageCropper';
      ...
      (file) => {
        ...
        if (accept.length > 0) {
          ...
          const isValidType = accept.some((acceptType) => {
            const normalizedAccept = acceptType.toLowerCase().replace('.', '');
    ...
    return (
      <>
        <div className={`space-y-1 ${className}`}>
          ...
          {!showProgress && !showImageCropper ? (
            ...
            >
              ...
              <div className="flex flex-col items-center space-y-3">
                ...
                >
                  <FiUpload
                    className={`w-6 h-6 ${isUploadComplete ? 'text-green-600' : 'text-primary'
                      }`}
                  />
                </div>
          ...
          {displayFiles.length > 0 && !showProgress && !showImageCropper && (
              ...
              <div className="space-y-2">
                {displayFiles.map((file, index) => (
                  <FileItem
                    ...
                    file={file}
                    onRemove={() => removeFile(multiple ? index : null)}
