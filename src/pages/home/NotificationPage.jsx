...
const NotificationPage = () => {
  ...
  const getNotificationStyle = (actionType, isRead) => {
    const baseStyle =
      'backdrop-blur-md bg-white/40 border border-white/20 shadow-lg';

    if (actionType === 'Approved') {
      return `${baseStyle} ${
        isRead
          ? 'border-l-4 border-l-green-400'
          : 'border-l-4 border-l-green-500 bg-white/50'
      }`;
    } else if (actionType === 'Declined') {
      return `${baseStyle} ${
        isRead
          ? 'border-l-4 border-l-red-400'
          : 'border-l-4 border-l-red-500 bg-white/50'
      }`;
    } else {
      return `${baseStyle} ${
        isRead
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 p-4 pb-6">
